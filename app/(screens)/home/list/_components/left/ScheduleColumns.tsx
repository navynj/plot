...
const ScheduleColumns = () => {
  ...
  const schedules = useMemo(() => {
    return todos?.filter((todo) => todo.scheduleStart || todo.scheduleEnd);
  }, [todos]);
  ...
  useEffect(() => {
    ...
    schedules?.forEach(({ scheduleStart }) => {
      if (scheduleStart?.time && scheduleStart.time <= today) {
        current = `schedule-column-${getTime(scheduleStart.time)}`;
      }
    });
