...
const TrackInputOverlay = () => {
  ...
  useEffect(() => {
    if (showOverlay) {
      ...
      if (trackData && trackData.date) {
        ...
        trackData.profile &&
          router.replace(
            `/home/list?track-input=show&trackId=${trackData.id}&profileId=${trackData.profile.id}`
          );

        if (albums?.find(({ id }) => id === albumId)?.title === trackData.title) {
          setIsAutoTitle(true);
        }

        form.setValue('title', trackData.title || '');
        form.setValue('profileId', trackData.profileId);
        // NOTE: profileId 기준으로 앨범 목록 필터링하여 렌더링됨. 해당 선택 목록 렌더링 이후에 albumId가 변경되어야 UI에 반영 가능
        setTimeout(() => {
          form.setValue('albumId', trackData.albumId);
        }, 100);
        form.setValue('content', trackData.content || '');
        setEmoji(
          trackData.icon || trackData.album?.icon || trackData.profile?.icon || ''
        );
  ...
  }, [titleInputValue, albumIdInputValue, profileId]);
