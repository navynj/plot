...
const [mood] = await sql`select id, child_schema from node where user_id=${u.id} and title='Mood' and deleted_at is null`;
...
const [exp] = await sql`select id, child_schema from node where user_id=${u.id} and title='Expense' and deleted_at is null`;
if (exp) {
...
console.log('Expense inOut:', JSON.stringify(exp?.child_schema && (await sql`select child_schema from node where id=${exp.id}`)[0].child_schema.find((d)=>d.key==='inOut')));
